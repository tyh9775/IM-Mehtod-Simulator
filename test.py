...
def bw_pdf(md,md0,mn,mpi,mwidth=None):
  A=0.95 
  if md==mn+mpi:
    q=0
  elif md<mn+mpi:
...
def generate_random_values(num_samples, x_values, y_values):
  # Create a cumulative distribution function (CDF) by integrating y_norm
  cdf = np.cumsum(y_values)
  # Normalize the CDF
  cdf /= cdf[-1]
  # Generate random numbers from a uniform distribution
  random_numbers = np.random.rand(num_samples)
  # Find the indices corresponding to the random numbers in the CDF
  indices = np.searchsorted(cdf, random_numbers)
  # Map the indices to the corresponding x_values
  generated_values = x_values[indices]
  ...
  for i in range(0,N_events):
    ...
    for j in range(0,N_delta):
      ...
      if Gaus is True:
        ...
        mdel=generate_random_values(1,x_bw,y_norm)
      #PID of delta:
      dpid=2224 #delta++
